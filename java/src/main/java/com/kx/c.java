...
public class c{
  ...
  */
  ...
  public c(String host,int port,String usernamepassword,boolean useTLS,int timeout) throws KException,IOException{
    wBuff=new byte[2+ns(usernamepassword)];
    s=new Socket();
    s.connect(new InetSocketAddress(host,port),Math.max(timeout,1000));
   ...
   */
  /**
   * {@code Projection} represents a kdb+ projection (partially applied function).
   * For example, {@code 1+} is the {@code +} function partially applied to {@code 1}.
   * The items array holds the projected arguments and the function as returned by the deserializer.
   */
  public static class Projection {
    public Object[] items;
    public Projection(Object[] items) { this.items = items; }
    public String toString() {
  ...
  Object r() throws UnsupportedEncodingException{
    ...
    if(t>99){
      ...
      if(t<104) {
        byte b = rBuff[rBuffPos++];
        if(t==101 && b==0) return null;
        return "func";  // primitive functions cannot be fully serialized
      }
      if(t>105)
        r();
      else {
        n=ri();
        Object[] projItems = new Object[n];
        for(i=0;i<n;i++) projItems[i]=r();
        return new Projection(projItems);
      }
      return "func";
    }
    if(t==99)
